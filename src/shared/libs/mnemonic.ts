...
export default class Mnemonic {
    ...
    private constructor(entropy: Buffer, words: string[], lan: string) {
        this._entropy = entropy;
        this._words = words;
        this._language = lan;
    }

    static genEntropy(bytes: number): Buffer {
        return crypto.randomBytes(bytes);
    }

    static generate(entropy: Buffer, lan: string): Mnemonic | null {
        ...
        const bits = ([] as number[]).concat(
            ...[...entropy].map(uint8ToBitArray)
        );
        ...
        for (let i = 0; i < bits.length / 11; i++) {
            ...
            words.push(wordlist[lan as string][idx.valueOf()]);
        }

        return new Mnemonic(entropy, words, lan);
    }

    static parse(phrase: string, lan: string): Mnemonic | null {
        const words = phrase.normalize("NFKD").split(" ");
        if (words.length % 3 !== 0) return null;

        const bitArrays: number[][] = [];
        for (let i = 0; i < words.length; i++) {
            const word = words[i.valueOf()];
            const idx = wordlist[lan as string].indexOf(word);
        ...
        return new Mnemonic(entropyBuf, words, lan);
    }
    ...
    get wordsIdx(): number[] {
        return this._words.map(word => wordlist[this._language].indexOf(word));
    }
    ...
    // async toSeedAsync(passphrase = ""): Promise<Buffer> {
    //     try {
    //         const salt = `mnemonic${passphrase}`;
    ...
    //     }
    // }
    toSeedAsync(passphrase = ""): Promise<Buffer> {
        const salt = `mnemonic${passphrase}`;
        return new Promise<Buffer>((resolve, reject) => {
            Mnemonic.pbkdf2(
                this.phrase.normalize("NFKD"),
